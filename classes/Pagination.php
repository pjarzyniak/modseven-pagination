 ...
 *
 * @package    Modseven/Pagination
 * @category   Base
 * @author     Paweł Jarzyniak <pjarzyniak@gmail.com>
 * @author     Koseven Team
...
class Pagination {
	...
	{
        // from extended class
        // TODO: modify to use Session, Request::query
        if(isset($_SESSION['pagination_perpage']) && (int)$_SESSION['pagination_perpage'] > 0)
        {
            $config['items_per_page'] = (int)$_SESSION['pagination_perpage'];
        }

        if(isset($_GET['perpage']) && (int)$_GET['perpage'] > 0)
        {
            $config['items_per_page'] = (int)$_GET['perpage'];
            $_SESSION['pagination_perpage'] = $config['items_per_page'];
        }
	...
	public function configGroup($group = 'default')
	{
		...
		$config_file = Config::instance()->load('pagination');
     ...
     * @throws Exception
     */
	public function url($page = 1) : string
	{
	 ...
	 * @since   3.0.7
	 */
	public function validPage($page) : bool
	{
	 ...
	 *
	 * @param	Request
	 * @return	Request|self	If used as getter
	 ...
	 *
	 * @param	Route
	 * @return	Route|self	Route if used as getter
	 * @return	$this	Chainable as setter
	 */
     ...
     * @return    array|self    Route parameters if used as getter
     */
	public function routeParams(array $route_params = NULL)
	{
	...
	public function __toString() : string
	{
		...
		{
		    Log::instance()->log(Log::ERROR, $e->getMessage().' '.$e->getTraceAsString());
			return '';
		}
	}
